...
///
/// Versioned via `DuplessMigrationPlan`, so schema changes migrate the user's
...
enum SharedStore {
    static let container: ModelContainer = {
        ...
        do {
            return try ModelContainer(for: schema, migrationPlan: DuplessMigrationPlan.self, configurations: [config])
        } catch {
            ...
            if let url = config.url as URL?, FileManager.default.fileExists(atPath: url.path) {
                ...
                if let restored = try? ModelContainer(for: schema, migrationPlan: DuplessMigrationPlan.self, configurations: [config]) {
                    return restored
                }
            }
